...
class PlgTZ_Portfolio_PlusContentNavigationModelArticle extends TZ_Portfolio_PlusPluginModelItem
{
    ...
    public function getNavigation(){
        ...
        if($row = $this -> getItem()){
            ...
            $query->select('a.id, a.title, m.catid, a.language,' . $case_when . ',' . $case_when1)
            ...
            if ($row->prev)
            {
                $row->prev_label = ($params->get('navigation_display', 0) == 0) ? JText::_('JPREV') : $row->prev->title;
                $row->prev = JRoute::_(TZ_Portfolio_PlusHelperRoute::getArticleRoute($row->prev->slug, $row->prev->catid, $row->prev->language));
            }
            else
            {
                $row->prev_label = '';
            ...
            if ($row->next)
            {
                $row->next_label = ($params->get('navigation_display', 0) == 0) ? JText::_('JNEXT') : $row->next->title;
                $row->next = JRoute::_(TZ_Portfolio_PlusHelperRoute::getArticleRoute($row->next->slug, $row->next->catid, $row->next->language));
            }
            else
            {
                $row->next_label = '';
