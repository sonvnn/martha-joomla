/**
 * Style UI for Grid Gallery Uploader
 */
...
jQuery.extend(GridGallery, {
    ...
    initUploader: function () {
        jQuery('#grid_gallery_files').sortable();
        jQuery('#grid_gallery_files').find('li.media').each(function (i, el) {
            ...
            uploader.find('.delete_grid_image').on('click', function (event) {
                ...
                if (confirm('Are you sure?')) {
                    ...
                    if (jQuery('#grid_gallery_files li.media').length <= 0) {
                        jQuery('#grid_gallery_files').find('li.empty').fadeIn();
                    }
                    GridGallery.ui_add_log('File \''+uploader.attr('data-name')+'\' deleted');
                }
            })
        })
    },
    ...
    ui_add_log: function (message, color) {
        ...
        var template = jQuery('#grid_gallery_debug_template').text();
        template = template.replace('%%date%%', dateString);
        template = template.replace('%%message%%', message);
        template = template.replace('%%color%%', color);

        jQuery('#grid_gallery_debug').find('li.empty').fadeOut(); // remove the 'no messages yet'
        jQuery('#grid_gallery_debug').prepend(template);
    },

    // Creates a new file and add it to our list
    ui_multi_add_file: function (id, file) {
        var uploader = jQuery('#grid_gallery_files_template').text();
        ...
        jQuery('#grid_gallery_files').prepend(uploader);
    },
    ...
    ui_multi_update_file_data: function (id, file) {
        ...
        jQuery('#grid_gallery_files').find("[data-name='"+file.name+"']").remove();
        uploader.find('.filename').text(file.name);
        uploader.attr('data-name', file.name);
        uploader.find('.grid_gallery_url').attr('value', file.name);
        uploader.find('.grid_image_featured').attr('value', file.name);
        if (jQuery('#grid_gallery_files').find('li.empty').is(':visible')) {
            uploader.find('.grid_image_featured').attr('checked','checked');
        }

        jQuery('#grid_gallery_files').find('li.empty').fadeOut(); // remove the 'no files yet'
        uploader.find('.delete_grid_image').on('click', function (event) {
            event.preventDefault();
            if (confirm('Are you sure?')) {
                uploader.remove();
                ...
                GridGallery.ui_add_log('File \''+file.name+'\' deleted');
            }
        });
    }
});
