...
?>
<div id="tpPortfolio" class="<?php echo $bootstrapClass;?>tpPortfolio tplSelena <?php echo $this->pageclass_sfx;?>">
                ...
                <div id="sort" class="option-set clearfix" data-option-key="sortBy">
                    ...
                    foreach($sortfields as $sortfield):
                        switch($sortfield):
                            case 'title':
                                ?>
                                <a class="btn btn-primary btn-sm<?php echo ($sort == 'alpha' || $sort == 'ralpha')?' active':''?>"
                                   href="#title" data-option-value="name"><?php echo JText::_('COM_TZ_PORTFOLIO_PLUS_TITLE');?></a>
                                <?php
                                break;
                            case 'date':
                                ?>
                                <a class="btn btn-primary btn-sm<?php echo ($sort == 'date' || $sort == 'rdate')?' active':''?>"
                                   href="#date" data-option-value="date"><?php echo JText::_('COM_TZ_PORTFOLIO_PLUS_DATE');?></a>
                                <?php
                                break;
                            case 'hits':
                                ?>
                                <a class="btn btn-primary btn-sm<?php echo ($sort == 'hits' || $sort == 'rhits')?' active':''?>"
                ...
                <div id="layouts" class="option-set clearfix" data-option-key="layoutMode">
                    ...
                    if(count($params -> get('layout_type',array('masonry','fitRows','straightDown')))>0):
                        foreach($params -> get('layout_type',array('masonry','fitRows','straightDown')) as $i => $param):
                            ?>
                            <a class="btn btn-primary btn-sm<?php if($i == 0) echo ' active';?>" href="#<?php echo $param?>" data-option-value="<?php echo $param?>">
                                <?php echo $param?>
                            </a>
                        <?php endforeach;?>
                    <?php endif;?>
