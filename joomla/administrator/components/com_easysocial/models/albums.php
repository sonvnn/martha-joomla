...
class EasySocialModelAlbums extends EasySocialModel
{
	...
	public function getAlbums($uid = '', $type = '', $options = array())
	{
		...
		$db = ES::db();
		$sql = $db->sql();
		$my = isset($options['viewer']) ? $options['viewer'] : ES::user();
		$streamLib = ES::stream();
		...
		if ($sort == 'likes') {
			$query .= ", (select count(1) from `#__social_likes` as exb where exb.`uid` = a.`id` and exb.`type` = 'albums.user.create') as likes";
		}
