...
class SocialConversation extends EasySocial
{
	...
	public function toExportData(SocialUser $viewer, $extended = false)
	{
		...
		$reciepients = $this->getParticipants(null);
		$users = array();

		foreach ($reciepients as $recipient) {
			$users[] = $recipient->toExportData($viewer);
		}
		...
		$canAddParticipant = $this->isWritable($this->my->id) && $this->access->allowed('conversations.invite') && !$this->isArchived($this->my->id) ? true : false;
		...
		$result = array(
			...
			'lastReplied' => $this->getLastRepliedDate(true),
			'lastMessage' => $lastMsg,
			'isUnread' => $this->isNew(),
		...
		if ($extended) {
			$messages = $this->getMessages(array('sort' => 'desc'), $viewer->id);
			$nextAvatar = false;
