			...
			if(isset($field->field_value) && is_array($field->field_value)){
				foreach($field->field_value as $val){
					if(!empty($selected) && is_array($selected) && in_array($val[0], $selected)){
						$html.=JText::_('HIKA_FILTER_LIST_PREPEND').'<a class="hikashop_filter_list_selected" style="font-weight:bold">'.JText::_($val[1]).'</a>'.JText::_('HIKA_FILTER_LIST_SEPARATOR');
						$values.=$val[0].',';
					}
				}
...
class hikashopCursorClass extends hikashopFilterTypeClass{
	...
	function display($filter, $divName, &$parent, $datas=''){
		...
		if(!empty($filter->filter_options['input'])) {
			...
			$onupdate .= "
if(handle)
	document.getElementById('slider_".$filter->filter_namekey.'_'.$divName."_end').value = values[handle];
else
	document.getElementById('slider_".$filter->filter_namekey.'_'.$divName."_start').value = values[handle];
			";
			$extrajs.="
