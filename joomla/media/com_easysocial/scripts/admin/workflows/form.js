...
EasySocial.Controller('Workflows', {
	...
	hasDuplicateUniqueField: function() {
		var elements = [];
		var stepContent = $(self.content);
		var fields = stepContent.find(self.field);

		$.each(fields, function(i, field) {
			var field = $(field);
			var isUnique = field.data('unique');

			// Skip it if the field is not unique
			if (!isUnique) {
				return;
			}

			var element = field.data('element');
			elements.push(element);
		});

		// Get the original length of the elements
		var elementsLength = elements.length;
		
		// Remove the duplicate values from this array to compare with elementsLength
		uniqueElements = $.unique(elements);
		
		// If the length of both are different, we know that there is duplicate unique fields in it
		if (elementsLength !== uniqueElements.length) {
			return true;
		}
	...
	validate: function() {
		...
		if (self.hasDuplicateUniqueField()) {
			self.showDialogError('has-duplicate-unique');
			return false;
		}
	...
	addField: function(element, field, wrapper) {
		...
		if (field == undefined) {
			...
			field.attr('data-unique', element.data('unique'));
