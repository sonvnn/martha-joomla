...
}),b.resolve()}),EasyBlog.require().library("dialog").script("admin/grid","shared/toggler","admin/vendors/select2").done(function(a){a("[data-table-grid-filter]").select2({theme:"backend",width:"resolve",minimumResultsForSearch:1/0}),a("[data-table-grid-filter]").on("select2:open",function(){a("body").addClass("has-select2-dropdown")}),a("[data-table-grid-filter]").on("select2:close",function(){a("body").removeClass("has-select2-dropdown")})}),EasyBlog.module("admin/grid",function(){var b=this;EasyBlog.Controller("Grid",{defaultOptions:{"{sortColumns}":"[data-table-grid-sort]","{ordering}":"[data-table-grid-ordering]","{direction}":"[data-table-grid-direction]","{task}":"[data-table-grid-task]","{searchInput}":"[data-table-grid-search-input]","{search}":"[data-table-grid-search]","{resetSearch}":"[data-table-grid-search-reset]","{publishItems}":"[data-table-grid-publishing]","{filters}":"[data-table-grid-filter]","{checkAll}":"[data-table-grid-checkall]","{row}":"tbody > tr","{checkbox}":"[data-table-grid-id]","{boxChecked}":"[data-table-grid-box-checked]","{checkboxWrapper}":"[data-table-grid-checkbox]","{attachFile}":"[data-attach-exported-templates-file]",overrideCheckbox:!0}},function(a,b){return{init:function(){a.implementSortable(),a.implementPublishing()
